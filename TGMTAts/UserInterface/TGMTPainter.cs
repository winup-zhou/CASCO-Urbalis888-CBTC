...
namespace TGMTAts {
    ...
    public static class TGMTPainter {
        ...
        private static Pen circlePenWhite = new Pen(Color.FromArgb(191, 190, 190), 6);
        private static Pen circlePenOrangeRed = new Pen(Color.OrangeRed, 6);
        private static Pen circlePenRed = new Pen(Color.Red, 6);
        ...
        private static Bitmap hmi, atoctrl, dormode, dorrel, drvmode, emergency, fault, departure, menu,
            selmode, sigmode, special, stopsig, num0, numn0, latestmsg, alive, numn1, num1, message, upbutton, downbutton, tdt_dmi, bmsel, rmsel, staname, ARsig, crewid,numn0b,tgspeed;
        ...
        public static void Initialize() {
            ...
            TDTstringFormat.Alignment = StringAlignment.Far;
            ...
            hmi = new Bitmap(Path.Combine(imgDir, "base.png"));
        ...
        public static void PaintHMI(TGMTAts.AtsIoArray panel, TGMTAts.AtsVehicleState state) {
            ...
            {
                g800.BeginGDI();
                g800.DrawImage(hmi, 0, 0, /*TGMTAts.panel_[43]*/0 * 600, 730);
                ...
                g800.DrawImage(fault, 502, 399, TGMTAts.panel_[30] * 64, 64);
                ...
                g800.DrawImage(atoctrl, 0, 400, TGMTAts.panel_[21] * 64, 64);
                ...
                if (Math.Abs(StationManager.NextStation.StopPosition - state.Location) < Config.DoorEnableWindow && TGMTAts.signalMode > 0 && state.Speed < 1 && TGMTAts.localised) {
                    ...
                    if (sectogo < -99)
                    {
                        g800.Graphics.DrawString((state.Time % 1000 < 500 ? "  " : "99"), new Font("Arial", 48, FontStyle.Bold, GraphicsUnit.Pixel), new SolidBrush(Color.FromArgb(191, 190, 190)), 755, 66, TDTstringFormat);
                        //g800.DrawImage(numn0, 710, 87, (state.Time % 1000 < 500 ? 9 : 10) * 18, 18);
                        //g800.DrawImage(numn0, 696, 87, (state.Time % 1000 < 500 ? 9 : 10) * 18, 18);
                    }
                    else if (sectogo > -1)
                    {
                        g800.Graphics.DrawString("0", new Font("Arial", 48, FontStyle.Bold ,GraphicsUnit.Pixel), new SolidBrush(Color.FromArgb(191, 190, 190)), 755, 66, TDTstringFormat);
                        //g800.DrawImage(numn0, 710, 87, (state.Time % 1000 < 500 ? 0 : 10) * 18, 18);
                        //g800.DrawImage(numn0, 696, 87, 10 * 18, 18);
                    }
                    else
                    {
                        g800.Graphics.DrawString(str, new Font("Arial", 48, FontStyle.Bold, GraphicsUnit.Pixel), new SolidBrush(Color.FromArgb(191, 190, 190)), 755, 66, TDTstringFormat);
                        //g800.DrawImage(numn0, 710, 87, D(-sectogo, 0) * 18, 18);
                        //g800.DrawImage(numn0, 696, 87, D(-sectogo, 1) * 18, 18);
                    }
                }
                ...
                Poc(281, 238, 145, 0, tSpeed), Poc(281, 238, 108, -4, tSpeed), Poc(281, 238, 108, 4, tSpeed)
                });
                    g800.Graphics.FillPolygon(Brushes.Red, new Point[] {
                Poc(281, 238, 125, 0, tSpeed), Poc(281, 238, 108, -8, tSpeed), Poc(281, 238, 108, 8, tSpeed)
                });
                }
